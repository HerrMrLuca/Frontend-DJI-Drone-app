...
export function useGMapCover () {
  ...
  function init2DPin (name, coordinates, color, data) {
  ...
  function initPolyline (name, coordinates, color, data) {
    const path = []
  ...
  function initPolygon (name, coordinates, color, data) {
    const path = []
    coordinates.forEach(coordinate => {
      path.push(new AMap.LngLat(coordinate[0], coordinate[1]))
    })
  ...
  function removeCoverFromMap (id) {
  ...
  function updatePinElement (id, name, coordinates, color) {
    const element = getElementFromMap(id)
    if (element) {
...
// # sourceMappingURL=use-g-map-cover.js.map
